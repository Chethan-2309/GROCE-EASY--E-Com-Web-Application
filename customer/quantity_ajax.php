...
if(isset($_GET['cart_id_increment'])){
    ...
    $stmt=$admin->ret("SELECT * FROM `cart` INNER JOIN `product` ON product.pid=cart.pid WHERE  `cartid`='$cart_id' AND `cid`='$cus_id'");
    $row=$stmt->fetch(PDO::FETCH_ASSOC);

 
    $item_price = $row['pprice'];
    $old_quantity = $row['quantity'];
    $new_quantity = $old_quantity + 1;
    
    $total = $item_price * $new_quantity;

    $stmt1=$admin->cud("UPDATE `cart` SET `quantity`='$new_quantity' WHERE  `cartid`='$cart_id' AND `cid`='$cus_id'",'updated');
}
...
<div class="row">
                   ...
                   $g_total=0;
                    ...
                    <div class="row border-top border-bottom">
                        ...
                        <div class="row main align-items-center">
                            ...
                            <div class="col-2"><img class="img-fluid" src="../admin/controller/<?php echo $row['pimage']?>" style="width: 200px;height: 100px;"></div>
                            <div class="col">
                               
                                <div class="row"><?php echo $row['pname']?></div>
                            </div>
                            <div class="col">
                            <div class="col"><?php echo $row['pprice']?></div>
                            <input type="hidden" id="stock" value="<?php echo $row['pstock']; ?>" >
                            <input type="hidden" id="qty" value="<?php echo $row['quantity']; ?>" >
                            </div>
                            <div class="col">
                ...
                <button onclick="increment(<?php echo $row['cartid']; ?>)" type="button"  class="plus"><i class="fa fa-plus" aria-hidden="true"></i></button>
                            ...
                            <div class="col">
                            <div class=""style="margin-left: 70px;"><a href="controller/delete_cat.php?cusid=<?php echo $row['cid']?>"><i class="fa fa-times fa-2x"  aria-hidden="true"></i></a></div>
                            </div>
                ...
                <div class="col-md-4 summary">
                    ...
                    if($stmt3->rowCount()>0){
                    ?>
                    <a href="check_out.php?cus_id=<?php echo $cus_id?>"><button class="btn">CHECKOUT</button></a>
                    <?php } else { ?>
